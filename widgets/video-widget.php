...
class Amplify_Video extends WP_Widget {
	...
	<p>
	<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'amplify'); ?></label>
	<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
	</p>

	<p><label for="<?php echo $this->get_field_id( 'url' ); ?>"><?php _e( 'Paste the URL of the video (only from a network that supports oEmbed, like Youtube, Vimeo etc.):', 'amplify' ); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['amplify_video']) )
			delete_option('amplify_video');		  
		  
		return $instance;
	}
	
	function flush_widget_cache() {
		wp_cache_delete('amplify_video', 'widget');
	}
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'amplify_video', 'widget' );
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'amplify_video', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
